/*************************************************************\
|
|  FullJoin.cs started on 6/7 by Nicholas Fullerton
|
|  This class represents a FULL OUTER JOIN SQL operation
| 
\**************************************************************/
...
namespace SQuilL {
   class FullJoin {
      ...
      public FullJoin(Table table1, Table table2, List<string> keys) {
         t1 = table1; t2 = table2;
         key1 = keys[0];
         key2 = keys[1];
         ...
         // add them in afterwards, then strip dupes and resize table:
      ...
      // This method adds rows from the right table,
      // in which there is no key match on the left table:
      public void addNoRightKeyMatch() {
         List<string> t2Keys = new List<string>();
         for (int i = 0; i < t1.getGrid().Rows.Count; i++) {
            t2Keys.Add(t1.getGrid().Rows[i].Cells[key1].Value.ToString());
         }
         ...
         for (int i = 0; i < t2.getGrid().Rows.Count; i++) {
            ...
            foreach (string key in t2Keys) {
               if (t2.getGrid().Rows[i].Cells[key2].Value.ToString() ==
                  key)
                  match = true;
            }
            ...
            if (!match) {
               ...
               List<string> partialRow = new List<string>();
               for (int n = 0; n < t1.getGrid().Columns.Count; n++) {
                  partialRow.Add("");
               }

               for (int k = 0; k < t2.getGrid().Columns.Count; k++) {
                  partialRow.Add(t2.getGrid().Rows[i].Cells[k].Value.ToString());
               }
